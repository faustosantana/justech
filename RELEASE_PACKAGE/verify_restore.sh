...
EXPECTED_COMMIT="9b440c3b51b9684bf48e6f31bbc23226b2994b6e"
...
if [[ "$HEAD" != "$EXPECTED_COMMIT" ]]; then
  echo "WARN: HEAD=$HEAD (expected $EXPECTED_COMMIT). Checking tag object only."
fi
[[ "$TAG_COMMIT" == "$EXPECTED_COMMIT" ]] || fail "tag $EXPECTED_TAG points to $TAG_COMMIT not $EXPECTED_COMMIT"
ok "tag $EXPECTED_TAG → $EXPECTED_COMMIT"
...
assert m["git"]["commit"] == "$EXPECTED_COMMIT"
...
# Critical conversational modules present at this commit
for f in \
