...
def ascii_filename_fallback(filename: str) -> str:
    ...
    clean = (filename or "download").replace("\\", "_").replace('"', "")
...
def build_content_disposition(disposition: str, filename: str | None = None) -> str:
    """
    Build Content-Disposition encodable in latin-1.

    Uses ASCII ``filename=`` plus RFC 5987 ``filename*=UTF-8''…`` for the real name.

    Accepts legacy one-arg calls ``build_content_disposition(filename)`` used by
    some DGCP/corporate download endpoints.
    """
    if filename is None:
        # One-arg legacy: first parameter is the filename.
        filename = disposition or "download"
        disposition = "attachment"
    disposition = disposition if disposition in ("inline", "attachment") else "attachment"
    if not filename or not str(filename).strip():
        filename = "download"
    clean = str(filename).replace('"', "").strip() or "download"
    # Ensure a usable extension when callers pass bare stems
