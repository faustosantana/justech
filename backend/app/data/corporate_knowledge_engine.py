...
class CorporateKnowledgeEngine:
    ...
    def get_company_profile(self, company_key: str = "justech") -> dict:
        from app.services.company_normalization_service import company_folder, normalize_company_key

        profile = {}
        key = normalize_company_key(company_key) or company_key.lower().replace(" ", "_")
        folder = company_folder(key)
        alt_paths = [
            f"00_DATOS_EMPRESAS/{folder}/datos_empresa.json",
            f"00_DATOS_EMPRESAS/{folder}/datos_empresa_{key}.json",
        ]
