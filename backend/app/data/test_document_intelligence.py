...
@pytest.mark.asyncio
async def test_completion_preview_sncc_f042():
    preview = await DocumentCompletionService().preview(form_type="SNCC.F042", company_key="justech")
    assert preview.form_type == "SNCC.F042"
    assert preview.fields.get("razon_social")
    assert preview.fields.get("rnc") == "131828061"
    assert "Francisco Segura Sandoval" in (preview.fields.get("direccion") or "")
