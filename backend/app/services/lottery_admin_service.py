...
from app.models.lottery import LotteryDraw, LotteryDrawNumber, LotteryLottery, LotteryUserFavorite
...
class LotteryAdminService:
    ...
    async def dashboard_v2(
        ...
        *,
        ...
        # Latest results from visible dashboard lotteries
        latest_rows = (
            await self.db.execute(
                select(LotteryLottery)
                .where(
                    ...
                    LotteryLottery.is_aggregate.is_(False),
                    LotteryLottery.last_draw_date.is_not(None),
                )
                .order_by(LotteryLottery.last_draw_date.desc(), LotteryLottery.display_order.asc())
                .limit(12)
            )
        ).scalars().all()
        latest_results = []
        for lot in latest_rows:
            card_nums: list[str] = []
            draw = (
                await self.db.execute(
                    select(LotteryDraw)
                    .where(LotteryDraw.lottery_id == lot.id, LotteryDraw.draw_date == lot.last_draw_date)
                    .limit(1)
                )
            ).scalar_one_or_none()
            if draw:
            ...
            latest_results.append(
                {
                    ...
                    "slug": lot.slug,
                    "date": lot.last_draw_date.isoformat() if lot.last_draw_date else None,
                    "numbers": card_nums,
                }
            )
