...
class HelleniaPaymentApplicationLine(models.Model):
    _name = "hellenia.payment.application.line"
...
class AccountPaymentApplication(models.Model):
    ...
    hellenia_application_line_ids = fields.One2many(
        "hellenia.payment.application.line",
        "payment_id",
        string="Detalle por factura",
        copy=False,
    )
    hellenia_show_application_detail = fields.Boolean(
        compute="_compute_hellenia_show_application_detail",
    )

    @api.depends(
        "hellenia_application_line_ids",
    ...
    def _compute_hellenia_show_application_detail(self):
        for pay in self:
            pay.hellenia_show_application_detail = bool(
                pay.hellenia_application_line_ids
                or pay.hellenia_applied_amount
                or pay.hellenia_withholding_total
                or pay.reconciled_invoice_ids
                or pay.reconciled_bill_ids
                or pay.state == "posted"
            )

    def _hellenia_applied_amount_for_invoice(self, move):
        ...
        if move in moves and len(moves) == 1 and self.hellenia_applied_amount:
            return self.hellenia_applied_amount
        if move in moves and len(moves) == 1:
            return self.amount
        return 0.0

    def _hellenia_reconciliation_label(self, move):
        labels = {
            "not_paid": "Sin pagar",
                ...
                AppLine.create(
                    {
                        ...
                        "reconciliation_state": pay._hellenia_reconciliation_label(move),
                    }
                )
