...
class HelleniaPaymentPartnerWizardLine(models.TransientModel):
    ...
    def _onchange_apply(self):
        ...
        for wiz in self.mapped("wizard_id"):
            wiz._forensic_log("line_onchange_apply")
    ...
    def _onchange_amount_to_pay(self):
        for line in self:
            if line.withholding_catalog_ids:
                line._recompute_line_withholdings()
        for wiz in self.mapped("wizard_id"):
            wiz._forensic_log("line_onchange_amount_to_pay")

    def write(self, vals):
        res = super().write(vals)
        for line in self:
            line.wizard_id._forensic_log("line_write", extra={"line_id": line.id, "vals": vals})
        return res
...
class HelleniaPaymentPartnerWizard(models.TransientModel):
    ...
    def create(self, vals_list):
        ...
        for wiz in wizards:
            ...
            wiz._forensic_log("after_create")
        return wizards

    def _forensic_line_snapshot(self):
        self.ensure_one()
        rows = []
        for line in self.line_ids.sorted("id"):
            sql_apply, sql_amount = None, None
            if isinstance(line.id, int):
                self.env.cr.execute(
                    """
                    SELECT apply, amount_to_pay
                    FROM hellenia_payment_partner_wizard_line WHERE id = %s
                    """,
                    [line.id],
                )
                row = self.env.cr.fetchone()
                if row:
                    sql_apply, sql_amount = row
            rows.append(
                {
                    "line_id": str(line.id),
                    "invoice_id": line.move_id.id,
                    "invoice": line.move_id.name,
                    "orm_apply": line.apply,
                    "sql_apply": sql_apply,
                    "orm_amount": line.amount_to_pay,
    ...
    def _forensic_log(self, stage, extra=None):
        self.ensure_one()
        invoice_ids = self.line_ids.mapped("move_id").ids
        _logger.warning(
            "%s [%s] wizard=%s partner=%s(%s) invoice_ids=%s line_count=%s lines=%s extra=%s",
            _FORENSIC,
            stage,
            self.id,
            ...
            invoice_ids,
            len(self.line_ids),
            self._forensic_line_snapshot(),
            extra,
        )

    def web_save(self, vals, specification=None, next_id=None):
        _logger.warning("%s [before_web_save] wizard=%s vals=%s", _FORENSIC, self.ids, vals)
        res = super().web_save(vals, specification, next_id)
        for wiz in self:
            wiz._forensic_log("after_web_save", extra={"vals": vals})
        return res
    ...
    def _selected_lines(self):
        ...
        selected = self.line_ids.filtered(lambda l: l.apply)
        self._forensic_log(
            "selected_lines",
            extra={"selected_ids": selected.ids, "selected_invoices": selected.mapped("move_id.name")},
        )
        return selected

    def action_register_payments(self):
        self.ensure_one()
        self._forensic_log("before_action_register_payments")
        ...
        for line in selected:
            ...
            _logger.warning(
                "%s [before_register_create] wizard=%s invoice=%s register_vals=%s applied=%s is_partial=%s",
                _FORENSIC,
                self.id,
                move.name,
                register_vals,
            ...
            _logger.warning(
                "%s [after_register_create] register_id=%s amount=%s custom_user_amount=%s diff_handling=%s",
                _FORENSIC,
                register.id,
                register.amount,
                register.custom_user_amount,
                register.payment_difference_handling,
            )
            ...
            _logger.warning(
                "%s [after_create_payments] invoice=%s payment_ids=%s payment_amounts=%s "
                "invoice_residual=%s invoice_payment_state=%s",
                _FORENSIC,
                move.name,
                created.ids,
                created.mapped("amount"),
