...
class JustechAdminHealthService(models.AbstractModel):
    ...
    def run_global_diagnostics(self):
        ...
        # System admin present
        sys_group = self.env.ref("base.group_system")
        admins = self.env["res.users"].sudo().search([("group_ids", "in", sys_group.id)])
        if not admins:
            findings.append(self._f("JAC_NO_SYSTEM_ADMIN", "critical", _("Sin Administrador del Sistema"), _("Debe existir al menos un administrador del sistema.")))

        jac_group = self.env.ref("justech_admin_center.group_justech_admin_center_manager")
        jac_managers = self.env["res.users"].sudo().search(
            [("group_ids", "in", jac_group.id)]
        )
        if not jac_managers and not admins:
            findings.append(self._f("JAC_NO_JAC_ADMIN", "critical", _("Sin Administrador Justech"), _("Asigne el rol Administrador Justech a un usuario.")))
        ...
        # Active without configuration signal
        for mod in self.env["justech.admin.module"].sudo().search([("functional_state", "=", "active"), ("technical_state", "=", "installed")]):
        ...
        for mod in self.env["justech.admin.module"].search([("technical_state", "=", "installed")]):
            self.run_module_health(mod, create_findings=True)
    ...
    @api.model
    def run_module_health(self, module, create_findings=True):
        module.ensure_one()
        summary = _("OK")
        severity = "info"
        if module.health_method:
            try:
                summary = self._invoke_health_method(module.health_method, module) or summary
            except Exception as exc:
                summary = str(exc)[:200]
                severity = "error"
                if create_findings:
                    self.env["justech.admin.health.finding"].sudo().create(
                        self._f(
                            "JAC_HEALTH_METHOD",
                            "error",
                            _("Health falló: %s") % module.functional_name,
                            summary,
                            module=module,
                        )
                    )
        module.write({"last_health_at": fields.Datetime.now(), "last_health_summary": summary})
        if severity == "error":
    ...
    def _invoke_health_method(self, dotted, module):
        # format: model.method  OR model.method with company
        ...
        # try with company if signature likely needs it
        try:
