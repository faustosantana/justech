...
export default async function globalSetup(_config: FullConfig) {
  ...
  if (tokens.tenant_id) {
    authHeaders["X-Tenant-Id"] = tokens.tenant_id;
  }

  const ctxRes = await fetch(`${API_BASE}/company-context`, {
    method: "PUT",
    headers: authHeaders,
    body: JSON.stringify({ selection_mode: "all" }),
  });
  if (!ctxRes.ok) {
    throw new Error(`Contexto multiempresa E2E falló: ${ctxRes.status} ${await ctxRes.text()}`);
  }
  ...
  if (!fs.existsSync(fixturesPath)) {
    throw new Error(
      `Faltan fixtures E2E en ${fixturesPath}. Ejecute make qa-e2e o el seed manual documentado en scripts/run_e2e_qa.sh`,
    );
  }
