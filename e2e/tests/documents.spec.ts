...
async function selectFixtureDocuments(page: import("@playwright/test").Page, fixtures: ReturnType<typeof loadFixtures>) {
  await selectRowsWithCheckboxes(page, fixtures.e2e_prefix, fixtures.document_ids.length, fixtures.document_ids);
}
...
test.describe("Documents bulk E2E", () => {
  test("selección, export, revisión, tarea, archivar y persistencia", async ({ page }, testInfo) => {
    ...
    await selectFixtureDocuments(page, fixtures);
    ...
    await page.locator("tr").filter({ hasText: `${fixtures.e2e_prefix}doc-1` }).locator('input[type="checkbox"]').check();
