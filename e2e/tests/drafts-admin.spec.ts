...
import { selectRowsWithCheckboxes } from "../helpers/select";
...
async function selectFixtureDrafts(page: import("@playwright/test").Page, fixtures: ReturnType<typeof loadFixtures>) {
  await selectRowsWithCheckboxes(page, fixtures.e2e_prefix, fixtures.draft_ids.length, fixtures.draft_ids);
}

async function selectBulkStatus(page: import("@playwright/test").Page, status: string) {
  await page.locator("main").getByText("Estado destino").locator("..").locator("select").selectOption(status);
}
...
test.describe("Price Drafts bulk E2E", () => {
  test("asignar, estado, tarea, export, descartar y persistencia", async ({ page }, testInfo) => {
    ...
    await selectFixtureDrafts(page, fixtures);

    await page.getByRole("button", { name: "Sin asignar" }).click();
    await page.getByPlaceholder("Buscar usuario…").fill("Marieli");
    await page.getByRole("button", { name: /Marieli marieli@justech/i }).click();
    await expect(page.getByTestId("bulk-action-assign")).toBeEnabled();
    ...
    await selectFixtureDrafts(page, fixtures);
    await selectBulkStatus(page, "listo_para_odoo");
...
test.describe("Admin empresas E2E", () => {
  test("cambiar empresas, default, guardar y persistencia API", async ({ page }, testInfo) => {
    ...
    for (const company of companies) {
      const shouldBeVisible = company.id === first.id || company.id === second.id;
      await page.getByTestId(`admin-company-check-${company.id}`).setChecked(shouldBeVisible);
    }
