...
export default function Table1Page() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const list = (tables.table1 || []) as MotorTableRow[];
      setRows(list);
      const n = Number(params.get("n"));
      if (Number.isInteger(n) && n >= 1 && n <= 100) {
        const hit = list.find((r) => r.number === n) || null;
        setDetail(hit);
      }
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Error al cargar Tabla 1");
    } finally {
      setLoading(false);
    }
  }, [params]);
  ...
  const onExport = async (format: "json" | "csv") => {
    try {
      const res = await apiClient.getLotteryNumericRelationsExport("table1", format);
  ...
  const onDetail = async (row: MotorTableRow) => {
    try {
      const full = await apiClient.getLotteryNumericRelationsNumber(row.number, "table1");
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">Motor Matemático · Tabla 1</h1>
      <p className="text-sm text-muted-foreground">Operación N ÷ 1220 · catálogo solo lectura</p>
