...
function SearchPageContent() {
  ...
  return (
    <AppShell title="Enterprise Search Center" description={m.search.description}>
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {result && result.resolved_entities && result.resolved_entities.length > 0 && (
          <Card className="border-emerald-500/30 bg-emerald-500/5">
            <CardContent className="py-3 text-sm">
              Entidad detectada:{" "}
              <strong>{result.resolved_entities[0].canonical_name}</strong>
              <span className="text-muted-foreground">
                {" "}
                ({result.resolved_entities[0].entity_type} · modo {result.search_mode ?? "hybrid"})
              </span>
            </CardContent>
          </Card>
        )}
            ...
            {result && (
              <p className="text-sm text-muted-foreground">
                ...
                {result.future_sources.length > 0 && (
                  <span className="ml-2 text-xs">({m.search.futureNote})</span>
                )}
              </p>
            )}
