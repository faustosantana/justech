...
export function buildHistoryBody(form: HistoryFormState): Record<string, unknown> {
  ...
  if (form.primaryIds.length < 1) {
    throw new Error("Selecciona lotería principal (visible, sin default oculto de lotería)");
  }
  const confirming =
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Tabla 1 = candidatos · Tabla 2 = confirmadores · Histórico por draw_id. Follow-up por
          defecto = misma lotería principal (visible abajo).
        </p>
      </CardHeader>
      <CardContent className="space-y-3 text-sm">
            ...
            {WINDOW_MODES.map((m) => (
              <label key={m} className="flex items-center gap-1 text-xs">
                <input
                  type="radio"
                  name="win"
                  checked={form.windowMode === m}
                  onChange={() => setForm((f) => ({ ...f, windowMode: m }))}
                />
                {m}
              </label>
            ))}
        ...
        <div>
          <div className="mb-1 font-medium">Lotería principal</div>
            ...
            {catalog.map((l) => (
              <label key={`p-${l.id}`} className="flex items-center gap-1 text-xs">
                <input
                  ...
                  onChange={() =>
                    setForm((f) => ({ ...f, primaryIds: toggle(f.primaryIds, l.id) }))
                  }
                />
                {l.name}
              </label>
        ...
        <div>
          ...
          <p className="mb-1 text-xs text-muted-foreground">
            Si no marcas ninguna, se usan las principales (explícito al enviar).
          </p>
          <div className="flex max-h-28 flex-wrap gap-2 overflow-y-auto rounded border p-2">
            {catalog.map((l) => (
        ...
        <div>
          <div className="mb-1 font-medium">Lotería de seguimiento (follow-up)</div>
          <label className="mb-2 flex items-center gap-2 text-xs">
            ...
            />
            Por defecto: misma lotería principal
...
export function RateCell({ rate }: { rate: Record<string, unknown> | null | undefined }) {
  ...
  return (
    <div className="text-xs">
      ...
      <div className="text-muted-foreground">n={String(rate.sample_size)} · {String(rate.sample_tier)}</div>
