#!/usr/bin/env bash
# Deploy DEV — sincroniza infra + custom desde Git y reinicia stack
set -euo pipefail
...
hellenia_log "DEV desplegado — commit: $(git -C "$REPO" rev-parse --short HEAD)" | tee -a "$LOG_FILE"
hellenia_log "URL: https://dev.hellenia.cloud" | tee -a "$LOG_FILE"
