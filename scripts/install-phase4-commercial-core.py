...
# stock puede auto-instalar stock_barcode; Fase 4 lo excluye explícitamente
barcode = env["ir.module.module"].search([("name", "=", "stock_barcode")], limit=1)
if barcode and barcode.state == "installed":
    print("[phase4] uninstalling auto-installed stock_barcode...")
...
bad = [m for m, s in forbidden_state.items() if s == "installed"]
if bad:
    print(f"[phase4] ERROR forbidden installed: {bad}")
else:
    print("[phase4] OK forbidden modules not installed")
