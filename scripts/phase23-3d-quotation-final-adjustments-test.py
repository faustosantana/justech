# -*- coding: utf-8 -*-
"""Fase 23.3D — Validación ajustes finales cotización Hellenia."""
...
OUT_DIR = os.environ.get(
    ...
    "/evidence/phase23-3d-quotation-final-adjustments",
)
if not os.path.isdir(os.path.dirname(OUT_DIR)) and not os.path.isdir("/evidence"):
    OUT_DIR = "/tmp/phase23-3d-quotation-final-adjustments"
os.makedirs(OUT_DIR, exist_ok=True)

report = {
    "phase": "23.3d-quotation-final-adjustments",
...
def render_screenshot(pdf_path, out_base):
    try:
        ...
        report.setdefault("screenshot_errors", []).append(str(exc)[:120])
        return None
...
def validate_html(html, label):
    ...
    for req in REQUIRED:
        safe = req.replace(" ", "_").replace("#", "")[:20]
        check(f"{label}_html_has_{safe}", req in html, req)
    check(f"{label}_html_payment_contado_or_credito",
          "Contado" in html or "Crédito" in html,
          "payment label")


def validate_pdf(text, label, page_count_hint=None):
    for bad in MOJIBAKE:
        check(f"{label}_pdf_no_mojibake", bad not in text, bad)
    for bad in FORBIDDEN_EN:
        check(f"{label}_pdf_no_{bad.replace(' ', '_')}", bad not in text, bad)
    for req in ("COTIZACIÓN", "República", "CONDICIONES", "Entregado por", "Recibido por"):
        check(f"{label}_pdf_has_{req[:12]}", req in text, req)
    check(f"{label}_pdf_payment_spanish",
          "Contado" in text or "Crédito" in text,
          text[:200])
...
for key, so in orders.items():
    ...
    report["pdfs"][key] = {
        ...
        "payment_term": so.get_hellenia_payment_term_display(),
    ...
    check(f"{key}_pdf_size", len(pdf_bytes) > 12000, len(pdf_bytes))
    if key == "quote_15":
        check("quote_15_pagination", len(pdf_bytes) > 50000 or "2" in pdf_text, len(pdf_bytes))
...
print("PHASE23_3D:" + json.dumps(report, indent=2, ensure_ascii=False))
