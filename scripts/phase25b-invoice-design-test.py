# -*- coding: utf-8 -*-
"""Fase 25B — Regenerar PDFs/PNG factura tras corrección diseño (solo TEST)."""
...
def render(key, move, pdf_name):
    ...
    result["files"][key] = {
        ...
        "document_type": move.get_jt_document_type_label(),
        "currency": move.get_jt_currency_display(),
...
inv_posted = Move.create({
    ...
    "partner_id": Partner.id,
    "invoice_date": datetime.now().date(),
    "invoice_line_ids": [line_cmd(p0, qty=1, price=1000)],
})
inv_posted.action_post()
render("invoice_fiscal_posted", inv_posted, "04_invoice_fiscal_b01.pdf")
