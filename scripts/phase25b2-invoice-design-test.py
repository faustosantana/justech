# -*- coding: utf-8 -*-
"""Fase 25B.2 — Evidencia visual factura alineada a cotización (solo TEST)."""
...
OUT_DIR = "/tmp/phase25b2-invoice-design"
os.makedirs(OUT_DIR, exist_ok=True)

REPORT_INV = "justech_report_design.report_justech_invoice_document"
REPORT_Q = "justech_report_design.report_hellenia_quotation_document"

result = {
    "phase": "25B.2-invoice-design",
...
def so_line_cmd(product, qty=1, price=100.0, discount=0.0):
    return (0, 0, {
        "product_id": product.id,
        "product_uom_qty": qty,
        "price_unit": price,
...
def render_inv(key, move, pdf_name):
    pdf_bytes, _ = Report._render_qweb_pdf(REPORT_INV, move.ids)
    ...
    result["files"][key] = {
        "pdf": pdf_name,
        "png": png_path,
        "invoice_number": move.get_jt_invoice_number_display(),
        "ncf": move.get_jt_ncf_display(),
    }


def render_q(pdf_name):
    so = SaleOrder.create({
        "partner_id": Partner.id,
        "order_line": [so_line_cmd(Product[0], qty=1, price=3500)],
    })
    pdf_bytes, _ = Report._render_qweb_pdf(REPORT_Q, so.ids)
    pdf_path = os.path.join(OUT_DIR, pdf_name)
    with open(pdf_path, "wb") as f:
        f.write(pdf_bytes)
    png_path = to_png(pdf_path, os.path.join(OUT_DIR, pdf_name.replace(".pdf", "")))
    result["files"]["quotation_reference"] = {"pdf": pdf_name, "png": png_path}


render_q("00_quotation_reference.pdf")
...
render_inv("invoice_1_line", inv1, "01_invoice_1_line.pdf")
...
render_inv("invoice_5_lines", inv5, "02_invoice_5_lines.pdf")
...
render_inv("invoice_with_discount", inv_disc, "03_invoice_with_discount.pdf")
