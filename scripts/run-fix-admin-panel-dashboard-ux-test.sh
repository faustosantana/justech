...
EVIDENCE="$PROJECT_ROOT/evidence/fix-admin-panel-dashboard-ux-test"
...
echo "=== FIX admin panel dashboard UX TEST ==="
...
docker compose --env-file ../../config/test/.env run --rm odoo odoo \
  -d hellenia_test --db_host=db --db_user="$DB_USER" --db_password="$DB_PASSWORD" \
  -u justech_modules,justech_admin --test-enable --stop-after-init 2>&1 | tee /tmp/dashux-tests.log | tail -8 || true
grep "tests.result" /tmp/dashux-tests.log || true
docker compose --env-file ../../config/test/.env up -d odoo
sleep 15
docker compose --env-file ../../config/test/.env exec -T odoo odoo shell \
  -d hellenia_test --db_host=db --db_user="$DB_USER" --db_password="$DB_PASSWORD" \
  --no-http < /tmp/validate-fix-admin-panel-dashboard-ux.py 2>&1 | tail -30
docker compose --env-file ../../config/test/.env exec -T odoo cat /var/lib/odoo/fix-admin-panel-dashboard-ux-validation.json > /tmp/dashux-validation.json
/opt/odoo-projects/hellenia/scripts/healthcheck.sh test 2>&1 | tee /tmp/dashux-hc-test.log | tail -8
REMOTE
scp -i "$SSH_KEY" "$VPS:/tmp/dashux-validation.json" "$EVIDENCE/validation.json" 2>/dev/null || true
scp -i "$SSH_KEY" "$VPS:/tmp/dashux-tests.log" "$EVIDENCE/tests.log" 2>/dev/null || true
scp -i "$SSH_KEY" "$VPS:/tmp/dashux-hc-test.log" "$EVIDENCE/healthcheck.log" 2>/dev/null || true
