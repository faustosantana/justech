...
docker compose --env-file ../../config/production/.env exec -T odoo cat /var/lib/odoo/fix-admin-panel-functions-validation.json > /tmp/adminpanel-prod-validation.json
/opt/odoo-projects/hellenia/scripts/healthcheck.sh prod 2>&1 | tee /tmp/adminpanel-hc-prod.log | tail -8
REMOTE
scp -i "$SSH_KEY" "$VPS:/tmp/adminpanel-prod-validation.json" "$EVIDENCE/validation.json" 2>/dev/null || true
scp -i "$SSH_KEY" "$VPS:/tmp/adminpanel-hc-prod.log" "$EVIDENCE/healthcheck.log" 2>/dev/null || true
echo "Backup: ${BACKUP}.dump"
