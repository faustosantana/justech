...
EVIDENCE="$PROJECT_ROOT/evidence/fix-client-modules-real-only-prod"
...
echo "=== FIX real-only client modules PROD ==="
...
BACKUP="\$PROJECT/backups/hellenia-prod/fix-client-modules-real-only-${TS}"
mkdir -p "\$BACKUP"
...
docker compose --env-file ../../config/production/.env exec -T odoo odoo shell \
  -d hellenia_prod --db_host=db --db_user="\$DB_USER" --db_password="\$DB_PASSWORD" \
  --no-http < /tmp/validate-fix-client-modules-real-only.py 2>&1 | tail -20
docker compose --env-file ../../config/production/.env exec -T odoo cat /var/lib/odoo/fix-client-modules-real-only-validation.json > /tmp/realonly-validation-prod.json
/opt/odoo-projects/hellenia/scripts/healthcheck.sh prod 2>&1 | tee /tmp/realonly-hc-prod.log | tail -8
REMOTE
scp -i "$SSH_KEY" "$VPS:/tmp/realonly-validation-prod.json" "$EVIDENCE/validation.json" 2>/dev/null || true
scp -i "$SSH_KEY" "$VPS:/tmp/realonly-hc-prod.log" "$EVIDENCE/healthcheck.log" 2>/dev/null || true
echo "backup=fix-client-modules-real-only-${TS}" > "$EVIDENCE/BACKUP.txt"
