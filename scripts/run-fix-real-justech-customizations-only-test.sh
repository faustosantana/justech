...
EVIDENCE="$PROJECT_ROOT/evidence/fix-real-justech-customizations-only-test"
...
echo "=== FIX real Justech customizations only TEST ==="
...
docker compose --env-file ../../config/test/.env run --rm odoo odoo \
  -d hellenia_test --db_host=db --db_user="$DB_USER" --db_password="$DB_PASSWORD" \
  -u justech_modules,justech_admin --test-enable --stop-after-init 2>&1 | tee /tmp/realonly2-tests.log | tail -5 || true
grep "tests.result" /tmp/realonly2-tests.log || true
...
docker compose --env-file ../../config/test/.env exec -T odoo cat /var/lib/odoo/fix-real-justech-customizations-only-validation.json > /tmp/realonly2-validation.json
/opt/odoo-projects/hellenia/scripts/healthcheck.sh test 2>&1 | tee /tmp/realonly2-hc-test.log | tail -8
REMOTE
scp -i "$SSH_KEY" "$VPS:/tmp/realonly2-validation.json" "$EVIDENCE/validation.json" 2>/dev/null || true
scp -i "$SSH_KEY" "$VPS:/tmp/realonly2-tests.log" "$EVIDENCE/tests.log" 2>/dev/null || true
scp -i "$SSH_KEY" "$VPS:/tmp/realonly2-hc-test.log" "$EVIDENCE/healthcheck.log" 2>/dev/null || true
