#!/usr/bin/env bash
# Fase 18.2 — Catálogo completo retenciones RD — TEST
set -euo pipefail
...
EVIDENCE="$PROJECT_ROOT/evidence/phase18-2-withholding-catalog-test.json"
LOG="$PROJECT_ROOT/logs/deploy/phase18-2-test-$(date +%Y-%m-%d_%H%M).log"
...
hellenia_log "=== Fase 18.2 Catálogo retenciones — TEST ===" | tee "$LOG"
...
"$SCRIPT_DIR/run-odoo-shell-env.sh" test phase18-2-validate-withholding-catalog-test.py PHASE18_2 "$EVIDENCE" 2>&1 | tee -a "$LOG"

"$SCRIPT_DIR/run-odoo-shell-env.sh" test phase18-validate-withholding-test.py PHASE18 \
  "$PROJECT_ROOT/evidence/phase18-2-phase18-regression.json" 2>&1 | tee -a "$LOG" || true

"$SCRIPT_DIR/run-odoo-shell-env.sh" test phase17-4-validate-payment-wizard-test.py PHASE17_4 \
  "$PROJECT_ROOT/evidence/phase18-2-wizard-regression.json" 2>&1 | tee -a "$LOG" || true

python3 -c "import json; d=json.load(open('$EVIDENCE')); exit(0 if d.get('ok') else 1)"

hellenia_log "PASS Fase 18.2 TEST — $EVIDENCE" | tee -a "$LOG"
