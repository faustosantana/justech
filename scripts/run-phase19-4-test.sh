...
LOG="${PROJECT_ROOT}/logs/phase19-4-test.log"
...
hellenia_log "Fase 19.4 — upgrade módulos"
...
docker compose --env-file "$ENV_FILE" run --rm -T \
  ...
  < "${SCRIPT_DIR}/phase19-4-validate-period-review-test.py" > "$TMP" 2>&1 | tee -a "$LOG"
...
if not payload.startswith('{'):
    ...
    sys.exit(1)
open('$EVIDENCE_DIR/phase19-4-period-review-test.json', 'w').write(json.dumps(json.loads(payload), indent=2, ensure_ascii=False))
