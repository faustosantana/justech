#!/usr/bin/env bash
# Fase 23.3E — Ajustes finos cotización — TEST
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase23-3e-quotation-final-polish"
...
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" /tmp/hellenia_reports.tgz \
  "${SCRIPT_DIR}/phase23-3e-quotation-final-polish-test.py" root@2.25.69.179:/tmp/
...
EVIDENCE=/opt/odoo-projects/hellenia/evidence/phase23-3e-quotation-final-polish
...
docker compose --env-file ../../config/test/.env exec -T odoo odoo shell \
  ...
  < /tmp/phase23-3e-quotation-final-polish-test.py > "$TMP" 2>&1
python3 -c "
d=open('$TMP').read(); m='PHASE23_3E:'; i=d.find(m)
...
"
docker cp hellenia-test-odoo-1:/tmp/phase23-3e-quotation-final-polish/. "$EVIDENCE/" 2>/dev/null || true
ls -la "$EVIDENCE"
REMOTE
