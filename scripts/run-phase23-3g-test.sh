#!/usr/bin/env bash
# Fase 23.3G — Ajuste quirúrgico CSS cotización — TEST
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase23-3g-quotation-surgical-css"
...
EVIDENCE=/opt/odoo-projects/hellenia/evidence/phase23-3g-quotation-surgical-css
...
if i<0: raise SystemExit('no marker')
r=json.loads(d[i+len(m):].strip())
r['phase']='23.3g-quotation-surgical-css'
...
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" -r \
  root@2.25.69.179:/opt/odoo-projects/hellenia/evidence/phase23-3g-quotation-surgical-css/* \
