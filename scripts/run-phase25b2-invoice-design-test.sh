#!/usr/bin/env bash
# Fase 25B.2 — Evidencia visual factura vs cotización (solo hellenia_test)
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase25b2-invoice-design"
...
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" /tmp/justech_report_design.tgz \
  "${SCRIPT_DIR}/phase25b2-invoice-design-test.py" root@2.25.69.179:/tmp/
...
docker compose --env-file ../../config/test/.env exec -T odoo odoo shell \
  ...
  < /tmp/phase25b2-invoice-design-test.py > /tmp/out-25b2.txt 2>&1
tail -5 /tmp/out-25b2.txt
EVIDENCE=/opt/odoo-projects/hellenia/evidence/phase25b2-invoice-design
mkdir -p "$EVIDENCE" && chmod 777 "$EVIDENCE"
docker cp hellenia-test-odoo-1:/tmp/phase25b2-invoice-design/. "$EVIDENCE/"
cp /tmp/out-25b2.txt "$EVIDENCE/shell.log"
REMOTE

scp -i "${HOME}/.ssh/hellenia_vps_ed25519" -r \
  root@2.25.69.179:/opt/odoo-projects/hellenia/evidence/phase25b2-invoice-design/* \
