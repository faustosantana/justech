#!/usr/bin/env bash
# Fase 26 — Backup PROD + factura oficial Justech + validación
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase26-official-invoice-prod"
...
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" /tmp/justech_report_design.tgz \
  "${SCRIPT_DIR}/phase26-official-invoice-prod.py" root@2.25.69.179:/tmp/
...
docker compose --env-file ../../config/production/.env exec -T odoo odoo shell \
  ...
  < /tmp/phase26-official-invoice-prod.py > /tmp/out-phase26-prod.txt 2>&1
tail -30 /tmp/out-phase26-prod.txt

EVIDENCE="${PROJECT}/evidence/phase26-official-invoice-prod"
mkdir -p "$EVIDENCE" && chmod 777 "$EVIDENCE"
docker cp hellenia-prod-odoo-1:/tmp/phase26-official-invoice-prod/. "$EVIDENCE/" 2>/dev/null || true
cp /tmp/out-phase26-prod.txt "$EVIDENCE/shell.log"
echo "${BACKUP_DIR}" > "$EVIDENCE/backup_path.txt"
echo "${TS}" > "$EVIDENCE/backup_timestamp.txt"
REMOTE

scp -i "${HOME}/.ssh/hellenia_vps_ed25519" -r \
  root@2.25.69.179:/opt/odoo-projects/hellenia/evidence/phase26-official-invoice-prod/* \
