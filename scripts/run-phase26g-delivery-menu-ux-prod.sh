#!/usr/bin/env bash
# Fase 26G — Backup PROD + reorganización menús Conduces
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase26g-delivery-menu-ux-prod"
mkdir -p "$EVIDENCE_DIR"

cd "$PROJECT_ROOT"
tar czf /tmp/phase26g_justech_report_design.tgz -C custom justech_report_design
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" /tmp/phase26g_justech_report_design.tgz \
  "${SCRIPT_DIR}/phase26g-delivery-menu-ux-prod.py" \
...
echo "phase=26g-delivery-menu-ux" >> "${BACKUP_DIR}/MANIFEST.txt"
...
tar xzf /tmp/phase26g_justech_report_design.tgz -C custom
chmod -R a+rX custom/justech_report_design
grep -q "19.0.6.2.0" custom/justech_report_design/__manifest__.py
...
docker compose --env-file ../../config/production/.env run --rm -T odoo odoo \
  ...
  -u justech_report_design --stop-after-init --no-http 2>&1 | tee /tmp/phase26g-upgrade.log | tail -18
...
docker compose --env-file ../../config/production/.env exec -T odoo odoo shell \
  ...
  < /tmp/phase26g-delivery-menu-ux-prod.py > /tmp/out-phase26g-prod.txt 2>&1 || true
tail -5 /tmp/out-phase26g-prod.txt

EVIDENCE="${PROJECT}/evidence/phase26g-delivery-menu-ux-prod"
mkdir -p "$EVIDENCE" && chmod 777 "$EVIDENCE"
docker cp hellenia-prod-odoo-1:/tmp/phase26g-delivery-menu-ux-prod/. "$EVIDENCE/" 2>/dev/null || true
cp /tmp/out-phase26g-prod.txt "$EVIDENCE/shell.log"
...
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" -r \
  root@2.25.69.179:/opt/odoo-projects/hellenia/evidence/phase26g-delivery-menu-ux-prod/* \
