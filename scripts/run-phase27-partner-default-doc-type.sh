#!/usr/bin/env bash
# Fase 27 — Tipo comprobante fiscal predeterminado en contactos: TEST luego PROD
set -euo pipefail
...
if [[ "$TARGET" == "test" ]]; then
  ...
  EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase27-partner-default-doc-type-test"
else
  ENV_FILE="config/production/.env"
  COMPOSE_DIR="docker/production"
  CONTAINER="hellenia-prod-odoo-1"
  EVIDENCE_DIR="$PROJECT_ROOT/evidence/phase27-partner-default-doc-type-prod"
fi

mkdir -p "$EVIDENCE_DIR"
cd "$PROJECT_ROOT"
tar czf /tmp/phase27_partner_doc_type.tgz \
  -C custom justech_l10n_do_base justech_l10n_do_ncf
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" /tmp/phase27_partner_doc_type.tgz \
  "${SCRIPT_DIR}/phase27-partner-default-doc-type-validation.py" \
...
if [[ "${TARGET}" == "prod" ]]; then
  ...
  echo "phase=27-partner-default-doc-type" >> "\${BACKUP_DIR}/MANIFEST.txt"
  echo "BACKUP_OK: \${BACKUP_DIR}"
fi

rm -rf custom/justech_l10n_do_base custom/justech_l10n_do_ncf
tar xzf /tmp/phase27_partner_doc_type.tgz -C custom
chmod -R a+rX custom/justech_l10n_do_base custom/justech_l10n_do_ncf
grep -q "justech_do_default_document_type_id" custom/justech_l10n_do_base/models/res_partner.py
...
docker compose --env-file ../../${ENV_FILE} run --rm -T odoo odoo \
  ...
  -u justech_l10n_do_base,justech_l10n_do_ncf --stop-after-init --no-http > /tmp/phase27-upgrade.log 2>&1
tail -10 /tmp/phase27-upgrade.log
...
docker compose --env-file ../../${ENV_FILE} exec -T odoo odoo shell \
  ...
  < /tmp/phase27-partner-default-doc-type-validation.py > /tmp/out-phase27-doc-type.txt 2>&1
tail -10 /tmp/out-phase27-doc-type.txt

EVIDENCE="\${PROJECT}/evidence/phase27-partner-default-doc-type-${TARGET}"
mkdir -p "\$EVIDENCE" && chmod 777 "\$EVIDENCE"
docker cp ${CONTAINER}:/tmp/phase27-partner-default-doc-type-${TARGET}/. "\$EVIDENCE/" 2>/dev/null || true
if [[ "${TARGET}" == "prod" ]]; then
...
scp -i "${HOME}/.ssh/hellenia_vps_ed25519" -r \
  root@2.25.69.179:/opt/odoo-projects/hellenia/evidence/phase27-partner-default-doc-type-${TARGET}/* \
