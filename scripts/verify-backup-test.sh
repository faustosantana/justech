#!/usr/bin/env bash
# Verifica que un backup TEST contiene todos los artefactos requeridos
# Uso: verify-backup-test.sh [ruta-backup-timestamp]
set -euo pipefail
...
BACKUP_DIR="${1:-}"
BACKUP_ROOT="${BACKUP_ROOT:-/opt/odoo-projects/hellenia/backups/test}"
FAIL=0
...
hellenia_log "=== Verificación backup TEST ==="
